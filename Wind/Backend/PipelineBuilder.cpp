...
namespace wind {
PipelineBuilder& PipelineBuilder::SetInputAssemblyState(vk::PrimitiveTopology topology,
...
PipelineBuilder& PipelineBuilder::SetRenderState(const Material::Desc& desc, uint32_t mrtCount) {
    ...
    m_attachmentState = std::vector<vk::PipelineColorBlendAttachmentState>(
        mrtCount, vk::PipelineColorBlendAttachmentState{
                      .blendEnable = blendEnable,
                      .colorWriteMask =
                          BLEND_ALL_FLAG}); // todo : change this state with different blend mode
    ...
    if (desc.rasterShader != nullptr) SetShaderState(*desc.rasterShader);
    return *this;
}
