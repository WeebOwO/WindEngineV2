...
namespace wind
{
    ...
    void
    CommandEncoder::Draw(uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance)
    {
        m_nativeHandle.draw(vertexCount, instanceCount, firstVertex, firstInstance);
    }

    void CommandEncoder::DrawIndexed(uint32_t indexCount,
    ...
    void CommandEncoder::BindVertexBuffer(uint32_t          firstBinding,
    ...
    void CommandEncoder::BindIndexBuffer(const vk::Buffer& buffer, vk::DeviceSize offset, vk::IndexType indexType)
    {
        m_nativeHandle.bindIndexBuffer(buffer, offset, indexType);
    }

    void CommandEncoder::SetViewport(const vk::Viewport& viewport) { m_nativeHandle.setViewport(0, 1, &viewport); }
    ...
    void CommandEncoder::TransferImageLayout(const vk::Image&                 image,
